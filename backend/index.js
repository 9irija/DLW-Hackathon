...
app.post('/review/start', async (req, res) => {
  try {
    ...
    REVIEW_SESSIONS.set(sessionId, {
      ...
      lastCompletedAgent: 'builder',
    });

    const partialReview = await finalize(payload, { builder: builderResult }, { runChallenges: false });
    ...
    return res.json({
      ...
      checkpoint: {
        ...
        message: 'Builder context has been prepared. Proceed to factchecker.',
      },
    });
  } catch (err) {
...
app.post('/review/next', async (req, res) => {
  try {
    ...
    const partialReview = await finalize(session.payload, session.agentResults, { runChallenges: false });
...
app.post('/review/finalize', async (req, res) => {
  try {
    ...
    const finalResult = await finalize(session.payload, session.agentResults, { runChallenges: true });
